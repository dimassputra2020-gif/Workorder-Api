   ...
   public function updateStatus(Request $request, $uuid)
{
    try {
        ...
        if (!$user) {
            $user = User::create([
                'name'      => $name,
                'npp'       => $npp,
                'ttd_path'  => $finalTtdMengetahui,
            ]);
        } else {
            if (!$user->ttd_path && $finalTtdMengetahui) {
                $user->update([
                    'ttd_path' => $finalTtdMengetahui,
                ]);
            }
        ...
        $finalTtdMengetahui = $user->ttd_path;

        $pengajuans->update([
            'mengetahui_name' => $user->name,
            'mengetahui_npp'  => $user->npp,
            'mengetahui_tlp'  => $tlp,
...

    {
        ...
        if ($user->ttd_path === $url) {
            ...
            if (file_exists(public_path($user->ttd_path))) {
                @unlink(public_path($user->ttd_path));
            }
        ...
        if ($index !== false) {
            ...
            if (file_exists(public_path($list[$index]))) {
                @unlink(public_path($list[$index]));
            }
